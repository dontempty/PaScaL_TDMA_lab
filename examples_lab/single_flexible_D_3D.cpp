...
// compile = g++ -O2 -std=c++17 -static-libgcc -static-libstdc++ -o single_D_3D.exe single_D_3D.cpp
// Run = ./single_D_3D.exe 
int main() {
    ...
    for (time=0; time<max_iter; ++time) {
        ...
        for (j=1; j<ny1-1; ++j) {
            ...
            for (i=1; i<nx1-1; ++i) {
                ...
                rhs_z[idx_kij(1, i, j, nz1, nx1)] += (coef_z_a) * (-coef_y_a)    * (-coef_x_a*z_left_bdy[idx_im] + (1.0-coef_x_b)*z_left_bdy[idx] - coef_x_c*z_left_bdy[idx_ip]);

                idx    = idx_ij(i  , j, nx1);
                idx_ip = idx_ij(i+1, j, nx1);
                idx_im = idx_ij(i-1, j, nx1);
                rhs_z[idx_kij(1, i, j, nz1, nx1)] += (coef_z_a) * (1.0-coef_y_b) * (-coef_x_a*z_left_bdy[idx_im] + (1.0-coef_x_b)*z_left_bdy[idx] - coef_x_c*z_left_bdy[idx_ip]);

                idx    = idx_ij(i  , j+1, nx1);
                idx_ip = idx_ij(i+1, j+1, nx1);
                idx_im = idx_ij(i-1, j+1, nx1);
                rhs_z[idx_kij(1, i, j, nz1, nx1)] += (coef_z_a) * (-coef_y_c)    * (-coef_x_a*z_left_bdy[idx_im] + (1.0-coef_x_b)*z_left_bdy[idx] - coef_x_c*z_left_bdy[idx_ip]);
                ...
                idx_ip = idx_ij(i+1, j-1, nx1);
                idx_im = idx_ij(i-1, j-1, nx1);
                rhs_z[idx_kij(nz1-2, i, j, nz1, nx1)] += (coef_z_c) * (-coef_y_a)    * (-coef_x_a*z_right_bdy[idx_im] + (1.0-coef_x_b)*z_right_bdy[idx] - coef_x_c*z_right_bdy[idx_ip]);

                idx    = idx_ij(i  , j, nx1);
                idx_ip = idx_ij(i+1, j, nx1);
                idx_im = idx_ij(i-1, j, nx1);
                rhs_z[idx_kij(nz1-2, i, j, nz1, nx1)] += (coef_z_c) * (1.0-coef_y_b) * (-coef_x_a*z_right_bdy[idx_im] + (1.0-coef_x_b)*z_right_bdy[idx] - coef_x_c*z_right_bdy[idx_ip]);

                idx    = idx_ij(i  , j+1, nx1);
                idx_ip = idx_ij(i+1, j+1, nx1);
                idx_im = idx_ij(i-1, j+1, nx1);
                rhs_z[idx_kij(nz1-2, i, j, nz1, nx1)] += (coef_z_c) * (-coef_y_c)    * (-coef_x_a*z_right_bdy[idx_im] + (1.0-coef_x_b)*z_right_bdy[idx] - coef_x_c*z_right_bdy[idx_ip]);
            }
        }
